...
const SearchBar = () => {
  ...
  return (
    <form onSubmit={submitSearch} className=" ">
      <div className="ml-20 mb-2 flex flex-col items-center">
        <h3 className=" mb-1 font-bold text-white text-1xl lg:text-2xl lg:mr-20 pr-[3rem] pb-5">
          Search Your Favorite Character
        </h3>

        <input
          className="bg-starYellow rounded text-spaceBlack font-bold border-2 border-spaceBlack focus:none  lg:ml-5 mr-[4rem]"
          type="text"
          value={personSearch}
          onChange={(e) => setPersonSearch(e.target.value)}
          placeholder=""
        />
        <button className="  bg-starYellow text-spaceBlack font-bold px-4 py-2 rounded-full baseline md:mb-0 hover:bg-spaceBlack my-5 mr-[4rem] hover:text-starYellow">
          Search
        </button>
      </div>
